...
export class VendorEventService {
    ...
    async getAllEvents(authId: number,): Promise<{ events: VendorEvent[]; totalItems: number; }> {
        try {
    ...
    async getEvent(authId: number, id: number): Promise<VendorEvent> {
        ...
        if (!vendorEvent) {
            throw new NotFoundException(`Event with ID ${id} not found or not associated with your vendor account`);
        }
    ...
    async create(userId: number, dto: CreateEventDto) {
        try {
            ...
            const startDate = moment(dto.start_date, ['DD/MM/YYYY', 'DD-MM-YYYY'], true);
            ...
            throw new InternalServerErrorException('Something went wrong while creating the vendor event.');
        }
    }
    ...
    async update(
        ...
        try {
            ...
            if (!existingEvent) {
                throw new NotFoundException('Event not found or you do not have permission to update it.');
            }
            ...
            throw new InternalServerErrorException('Something went wrong while updating the event.');
        }
    }
